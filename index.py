    ...
    try:
        if request.method == 'POST' or request.method == 'GET':
            ...
            if request.method == 'POST':
                ...
                Usuario = cur.fetchall()
                if len(Usuario) > 0:
                    if not 'idUser' in session and not 'NomUser' in session:
                        for row in Usuario:
        ...
        if 'idUser' in session and 'NomUser' in session:
            if 'idUser' in session:
                ...
                if len(datos_usuario) > 0:
                    for dato_usuario in datos_usuario:
                        id_tema = dato_usuario[4]
                    ...
                    if len(calificaciones) <= 5:
                        ...
                        for resultado in resultados:
                            ...
                            if perfil == "Reflexivo":
                                ...
                                sec_glo = "Secuencial"
                        sql = """SELECT * FROM [dbo].[objeto] WHERE act_ref = '""" + act_ref + """' and sen_int = '""" + sen_int + """' and vis_ver = '""" + vis_ver + """' and sec_glo = '""" + sec_glo + """' And id_tema = """ + str(id_tema) + """ and id_contexto in(""" + lista + """)
                                        Union all
                                        SELECT * FROM [dbo].[objeto] WHERE act_ref = '""" + act_ref + """' and sen_int = '""" + sen_int + """' and vis_ver = '""" + vis_ver + """' and sec_glo = '""" + sec_glo + """' And id_tema = """ + str(id_tema) + """
                                        Union all
                                        SELECT * FROM [dbo].[objeto] WHERE act_ref = '""" + act_ref + """' and vis_ver = '""" + vis_ver + """' and sec_glo = '""" + sec_glo + """' And id_tema = """ + str(id_tema) + """
                                        Union all
                                        SELECT * FROM [dbo].[objeto] WHERE vis_ver = '""" + vis_ver + """' and sec_glo = '""" + sec_glo + """' And id_tema = """ + str(id_tema) + """
                                        Union all
                                        SELECT * FROM [dbo].[objeto] WHERE vis_ver = '""" + vis_ver + """' And id_tema = """ + str(id_tema) + """
                                        Union all
                                        SELECT * FROM [dbo].[objeto] WHERE id_tema = """ + str(id_tema)
                        cur.execute(sql)
